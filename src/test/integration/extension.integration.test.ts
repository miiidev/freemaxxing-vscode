...
suite('FreeMaxxing extension (integration)', () => {
  ...
  suiteSetup(async function () {
    ...
    // freeMaxxing or anything else already owns the port, fail fast.
    try {
      await mockServer.listen(MOCK_SERVER_PORT, MOCK_SERVER_HOST);
    } catch (err: any) {
      assert.fail(
        `Could not bind mock server to ${MOCK_SERVER_HOST}:${MOCK_SERVER_PORT} ` +
          `(is a real freeMaxxing already running?). ${err?.message ?? err}`
      );
    }

    extension = vscode.extensions.getExtension('miiidev.freemaxxing-vscode');
    assert.ok(extension, 'extension miiidev.freemaxxing-vscode not found');
  ...
  test('registers all Phase 0/1 commands', async () => {
    ...
    const expected = [
      ...
      'freemaxxing.trace',
      'freemaxxing.revive',
      'freemaxxing.openConfig',
      'freemaxxing.copyEndpoint',
      'freemaxxing.exportStats',
    ];
  ...
  test('Copy Endpoint Config writes expected JSON to the clipboard', async function () {
    this.timeout(10_000);
    await vscode.commands.executeCommand('freemaxxing.copyEndpoint');
  ...
  test('health check observes the mocked server (status comes up)', async function () {
    ...
    // probed /v1; we assert the server itself answers like freemaxxing does.
    await new Promise((resolve) => setTimeout(resolve, 2600));
    const res = await fetch(`http://${MOCK_SERVER_HOST}:${MOCK_SERVER_PORT}/v1`);
    assert.strictEqual(res.status, 200);
    assert.strictEqual(res.headers.get('x-freemaxxing-served-by'), 'groq::gpt-oss-120b');
  });

  test('freemaxxing.status opens the dashboard view (Phase 1 behavior)', async function () {
    this.timeout(10_000);
    await vscode.commands.executeCommand('freemaxxing.status');
